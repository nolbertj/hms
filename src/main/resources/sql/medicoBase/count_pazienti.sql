with origin as (select mb.id as this_medico from medico_base mb WHERE mb.id=?),  
    origin2 as (select pm.id_paziente, pm.id_medico_base from paziente_medico pm JOIN origin o on o.this_medico=pm.id_medico_base
                WHERE pm.al is null),
...
select o2.id_medico_base, count(o2.id_paziente) as numero_pazienti from origin2 o2
    join paziente p on o2.id_paziente=p.id
