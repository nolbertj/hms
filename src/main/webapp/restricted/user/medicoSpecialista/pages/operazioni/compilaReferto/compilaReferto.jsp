...
<c:url var="absolutePath" value="/restricted/user/medicoSpecialista/pages/operazioni/compilaReferto" scope="page"/>
            ...
            <c:choose>
                <c:when test="${alertMSG eq true}">
                    <custom:alert typeAlert="error"
                                  message="La tua area di specializzazione non corrisponde con l'area dell'esame selezionato.">
                    </custom:alert>
                </c:when>
                ...
                <c:when test="${warningMSG eq true}">
                    <custom:alert typeAlert="warning"
                                  message="Referto non generato. Errore database">
                    </custom:alert>
                </c:when>
            </c:choose>
            <%-------------------------------------------------------------------------------------------%>
            <form action="${cp}/${compilaRefertoURL}" method="POST" id="compilaRefertoForm">
                ...
                <div class="form-row justify-content-between">
                    ...
                    <div class="form-group col-lg-6 flex-column">
                        <label for="medicoPrescrivente">Medico prescrittore</label>
                        <input type="text" class="form-control" id="medicoPrescrivente" readonly>
                    </div>
                    <div class="form-group col-lg-auto flex-column">
